...
################
#### VERSION DU 29 mai 2020
#### Édition spéciale du confinement 4
#### Ajout d'une option pour le rayonnement
#### 17 juin: modification de la loi de rayonnement
...
class VocalTractLumpedParameter(VocalTractBase):
    ...
    def __init__(self, label="vocal_tract_rho_V", N=2, ADD_RAY=True,**kwargs):
        ...
        # Equations
        equations = VocalTractEquations(N=N, ADD_RAY=ADD_RAY)

        # Composants stockants
        H = equations.Ham_cstrd
        X = equations.X
        if ADD_RAY:
            X.remove(equations.rho_vec[-1])
            X.remove(equations.Pi_y_vec[-1])
            X.remove(equations.nuR_vec[-1])
            X.remove(equations.vol_vec[-1])
        ...
        if ADD_RAY:
            ind = 2*N - equations.N_lambda -1
            equations.Jyx.col_del(ind)
            equations.Jyx.row_del(1)
            # equations.Jyx = equations.Jyx[::-2,::-2]
            equations.Jyx.row_del(-1)
            equations.Jyx.col_del(-1)
            equations.Jyx.col_del(-1)
            equations.Jyx.col_del(-1)
        display(equations.Jyx)
        ...
        if ADD_RAY:
            self.subs.update(equations.ray_subs)
...
class VocalTractEquations():
    ...
    def __init__(self, N=2, ADD_RAY=True):
        self.VERBOSE = True
        self.ADD_RAY = ADD_RAY
        # --- Constantes --- #
        ...
        # ---- Observateurs
        # RHS
        self.O = sy.Matrix(self.RHS_vec)
        for i in range(self.N_lambda):
            self.observers[self.O[i]] = self.update_rhs_eq[i]
    ...
    def create_symbols(self):
        ...
        # ------ Observateurs -------
        # Ce bloc est juste là pour ranger les observateurs dans un certain
        # ordre
        for i in range(self.N):
            self.mu(i+1)
        for i in range(self.N_lambda):
            self.mu_add_mu(i+1, i+2)
        for i in range(self.N_lambda):
            self.mu_minus_mu(i+1, i+2)
        
        # Right Hand Side of the equation on the algebraic constrained states
        self.RHS_vec = []
        for i in range(self.N_lambda):
            self.RHS_vec.append(self.rhs(i+1))
    ...
    def compute_cstrnd_Ham(self):
        ...
        # En cas de rayonnement, toutes les variables du dernier tube 
        # sauf nu_L sont à zéro
        if self.ADD_RAY:
            self.ray_subs = [
                    (self.rho_vec[-1], 0),
                    (self.nuR_vec[-1], 0),
            ...
            self.Ham_cstrd = self.Ham_cstrd.subs(self.ray_subs)
    ...
    def compute_Jxx(self):
        ...
        # If we add a tract for radiation, we need to 
        # delete the lines that corresponds to the 
        # useless components: rho_Ray, V_Ray, nu_RRay, Piy_Ray
        if self.ADD_RAY:
            # last Nu (hyp: fluid velocity is zero far away)
            ind = 2 * self.N - self.N_lambda -1
            self.Jxx.col_del(ind)
            self.Jxx.row_del(ind)
            cpt_del = 1

            # last Piy (hyp: room is not moving)
            ind = 3 * self.N - self.N_lambda -1 - cpt_del
            self.Jxx.col_del(ind)
            self.Jxx.row_del(ind)
            cpt_del += 1

            # last V:
            ind = 4 * self.N - self.N_lambda -1 - cpt_del
            self.Jxx.col_del(ind)
            self.Jxx.row_del(ind)
            cpt_del += 1

            # last rho:
            ind = 5 * self.N - self.N_lambda -1 - cpt_del
            self.Jxx.col_del(ind)
            self.Jxx.row_del(ind)
    ...
    def compute_Jwx(self):
        ...
        if self.ADD_RAY:
            self.Jwx = sy.zeros(1, self.Nxi*self.N-self.N_lambda-4)
            self.Jwx[0, 2*self.N - self.N_lambda - 2] = 1
        print(self.Jwx)
    ...
    def compute_zw(self):
        ...
        surf =  self.vol(self.N-1) / (2*self.ell(self.N-1))
        q = (Abs(self.w_turb) + self.w_turb)/2
        rho = self.rho_0 + self.rho(self.N-1)
        self.zturb = 1/(2*rho) * (q/surf)**2
    ...
    def rhs(self,ind):
        ''' 
        Returns a symbols that looks like "rhs_ind" 
        which will be calculated separetely. It is stored as an observer
        '''

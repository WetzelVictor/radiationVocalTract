...
def prtVerbose(msg, lvl):
    if VERBOSE >= lvl:
        print(msg)
class RadiationBase(pyphs.Core):
    pass

class radiation(RadiationBase):
    def __init__(self, label="radiation_prototype", **kwargs):
        super().__init__(label=label)
        equations = VocalTractEquations(N=2)

        # On prend les bons états, et on met à zéro ceux que l'on élimine (dans
        # le Hamiltonien)
